...
./cloudfuse mount $mntPath --config-file=$fileConfigPath --tmp-path=$tmpPath --file-cache-timeout=0
sleep 3
...
do
    ...
    rm -rf $tmpPath/*
    ./cloudfuse mount $mntPath --config-file=$v2configPath --tmp-path=$tmpPath --file-cache-timeout=0
    if [ $? -ne 0 ]; then
        exit 1
    fi
...
do
    ...
    while IFS=, read -r thread block file; do
        ...
        echo "cloudfuse Run with $thread threads, $block block size, $file file size"
...
# very slow and will not be a good test case for cloudfuse.
...
#     for file in $(cat ./test/scripts/fio_tests.csv  | cut -d "," -f3 | tail -n +3 | sort -u);
#     do
#         echo "cloudfuse Run with $block block size, $file file size"
